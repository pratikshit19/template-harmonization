/**
 * excel-export.js
 * Generates the 9-sheet CLM configuration and template harmonization report using SheetJS.
 * Aligned with the legal harmonization procedure:
 * 1. Summary & Reduction
 * 2. Clause Inventory
 * 3. Harmonization Matrix
 * 4. Redundancy Report
 * 5. Clause Library
 * 6. Smart Tags
 * 7. Assembly Logic
...
const ExcelExport = (() => {
  ...
  /**
   * @param {Array}  sectionGroups     - From SectionDetector
   * @param {Array}  harmonizedResults - From Harmonizer (standard + variations)
   * @param {Array}  docNames          - All document names
   * @param {Object} annotations       - From Harmonizer.annotateAll() { groupName: {smartTags, cliCandidates, assemblyLogic} }
   * @param {Array}  clauseInventory   - Flat list of decomposed clauses
   * @param {Object} similarityData    - Section similarity scores
   */
  function generate(sectionGroups, harmonizedResults, docNames, annotations = {}, clauseInventory = [], similarityData = {}) {
    const wb = XLSX.utils.book_new();

    // ── Sheet 1: Summary & Reduction ──────────────────
  ...
  function download(wb, filename) {
    // Use FileSaver for reliable filenames
