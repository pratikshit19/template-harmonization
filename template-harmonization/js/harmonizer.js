/**
 * harmonizer.js
 * Orchestrates AI annotation (smart tags, CLIs, assembly logic)
 * and harmonization (standard clause + variations) for all section groups.
 *
 * Flow per SOP: Annotate → then Harmonize
 */

const Harmonizer = (() => {

  /**
   * PHASE 1: Annotate all section groups (smart tags, CLIs, assembly logic).
   * Runs BEFORE harmonization per the SOP.
   *
   * @param {Array} sectionGroups - From SectionDetector.groupSections()
   * @param {Object} existingAnnotations - Pre-computed annotations from inline actions
   * @param {Function} onProgress - Called with (currentIndex, total, groupName)
   * @returns {Object} annotations - { groupName: { smartTags, cliCandidates, assemblyLogic } }
   */
  async function annotateAll(sectionGroups, existingAnnotations = {}, onProgress) {
    ...
    for (let i = 0; i < total; i++) {
      ...
      // Reuse existing inline annotations if available
      ...
      try {
        ...
        const result = await AIEngine.annotateSection(group.groupName, variants, AppState.excelSmartTags || []);
  ...
  /**
   * PHASE 2: Harmonize all section groups.
   * Uses annotations from Phase 1 to inform harmonization.
   *
   * @param {Array} sectionGroups - From SectionDetector.groupSections()
   * @param {Object} annotations  - From annotateAll()
   * @param {Array} existingResults - Pre-computed harmonizations from inline actions
   * @param {Function} onProgress - Called with (currentIndex, total, groupName)
   * @returns {Array} harmonized results
   */
  async function harmonizeAll(sectionGroups, annotations, existingResults = [], onProgress) {
    ...
    for (let i = 0; i < total; i++) {
      ...
      // Reuse existing inline results if available
      ...
      try {
        ...
        if (variants.length === 1) {
          // Only one source — use as-is, it's the standard
          result = {
        ...
        const fullResult = {
          ...
          sources: group.sections.map(s => s.docName),
          ...result,
          // Keep backward compat with 'harmonized' key
        ...
        const errResult = {
          groupName: group.groupName,
          error: true, // Keep the error flag so we know it failed
          standardClause: group.sections[0].content, // Fallback to first document
  ...
  /**
   * Build a plain-text document from harmonized sections
   * (includes standard clauses + variations)
   */
  ...
  /**
   * Generate and download a proper .docx Word document
   * using the docx library (loaded via CDN).
   */
  async function downloadAsDocx(harmonizedResults, filename = 'harmonized-template.docx') {
    ...
    for (let i = 0; i < harmonizedResults.length; i++) {
      ...
      // Standard clause text — split by newlines for proper paragraphs
    ...
    // Use FileSaver for reliable cross-browser download with proper filename
  ...
  /**
   * Trigger download as plain text (fallback)
   */
